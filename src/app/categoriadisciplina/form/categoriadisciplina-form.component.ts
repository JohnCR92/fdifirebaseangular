...
@Component({
    ...
    templateUrl: './categoriadisciplina-form.component.html',
    styleUrls: ['./categoriadisciplina-form.component.css']
})

export class CategoriaDisciplinaFormComponent {
    ...
    guardar() {
        ...
        this.categoriadisciplinaService.saveFdiCategoriaDisciplina(this.fdiCategoriaDisciplina).subscribe(fdiCategoriaDisciplina => {
            ...
            this.alertaService.mostrarAlertaInfo("Actualizando", "Se a guardado la Categoria con su respectiva Disciplina Correctamente!!")
            window.history.back();
    ...
    public saveCode(e): void {
        let categoriaId = e.target;
        let list = this.fdiCategorias.filter(x => x.categoriaId === categoriaId)[0];
